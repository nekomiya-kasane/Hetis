...
namespace Yuki::Detail {
    ...
    namespace {
        ...
        SubclassSnapshot* AllocSubclassSnapshot(std::size_t count) noexcept {
            ...
            snap->count = count;
            snap->data  = reinterpret_cast<const MetaCore* const*>(
                static_cast<unsigned char*>(mem) + sizeof(SubclassSnapshot));
            return snap;
        }
    } // namespace
    ...
    void InstallKernel(const MetaCore* core, MetaLinks* links,
        ...
        if (prior) {
            for (std::size_t k = 0; k < implementsCount; ++k) {
                ...
                if (!match) continue;
                if (match->providerClass == core) continue;       // benign re-install.
                if (match->seal.final) std::abort();              // D7 Final.
                if (info.flags.unique) std::abort();              // D7 Unique.
                if (match->seal.important && info.flags.important) std::abort();  // D7.2.
            }
        }
        ...
        if (prior) {
            for (std::size_t j = 0; j < prior->count; ++j) {
                if (prior->entries[j].providerClass == core) continue;  // replaced by this install.
        ...
        const MergedDispatchSnapshot* priorMerged   =
        ...
        if (priorDispatch) {
            ...
            RetireSnapshot(const_cast<DispatchEntry*>(priorDispatch->entries),
                           &DeleteDispatchEntryArray);
        ...
        if (priorMerged && priorMerged != reinterpret_cast<const MergedDispatchSnapshot*>(priorDispatch)) {
            RetireSnapshot(const_cast<MergedDispatchSnapshot*>(priorMerged),
                           &DeleteMergedDispatchSnapshot);
        }
    ...
    void AppendSubclassToBase(MetaLinks* baseLinks, const MetaCore* subCore) noexcept {
        if (!baseLinks || !subCore) return;
        std::lock_guard<std::mutex> g(baseLinks->writerMu);

        const SubclassSnapshot* prior =
            baseLinks->subclassedBy.load(std::memory_order_acquire);
        const std::size_t priorCount = prior ? prior->count : 0;

        // Idempotency check.
        for (std::size_t i = 0; i < priorCount; ++i) {
            if (prior->data[i] == subCore) return;
        }
