...
namespace Yuki {
    ...
    namespace Detail {
        ...
        [[nodiscard]] constexpr std::uint64_t IidHi(Iid iid) noexcept {
        ...
        [[nodiscard]] constexpr std::uint64_t IidLo(Iid iid) noexcept {
     ...
     */
    [[nodiscard]] inline std::optional<const DispatchEntry*> Probe(FingerprintCache& c, Iid iid,
                                                                   std::uint64_t liveEpoch) noexcept {
    ...
    inline void Publish(FingerprintCache& c, Iid iid, const DispatchEntry* entry, std::uint64_t epoch) noexcept {
        std::uint32_t idx = c.writeIdx.fetch_add(1, std::memory_order_relaxed) % FingerprintCache::kSlots;
