 ...
 *     - explicit ComPtr(T*)  — bumps the refcount (defensive, for aliasing an existing +1).
 *     - static Adopt(T*)     — consumes an existing +1 without bumping (the usual factory entry point).
...
namespace Yuki {
    ...
    class ComPtr {
      ...
      public:
        ...
        /// Copy-and-swap assignment: covers both copy and move assignment safely.
