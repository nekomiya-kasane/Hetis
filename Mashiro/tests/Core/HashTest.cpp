...
TEST_CASE("Concept: StatelessAlgo satisfied by algorithm structs", "[Core.Hash]") {
...
TEST_CASE("Concept: StatefulAlgo satisfied by state structs", "[Core.Hash]") {
...
TEST_CASE("Concept: AnyAlgo covers both", "[Core.Hash]") {
    STATIC_REQUIRE(Mashiro::Hashing::AnyAlgo<Fnv1a64>);
    STATIC_REQUIRE(Mashiro::Hashing::AnyAlgo<Fnv1a64State>);
}

TEST_CASE("Concept: ResultOf yields correct types", "[Core.Hash]") {
...
TEST_CASE("Concept: Hashable satisfied by various types", "[Core.Hash]") {
...
TEST_CASE("Stateful matches stateless for string", "[Core.Hash]") {
...
TEST_CASE("Stateful chunked feed equals whole feed", "[Core.Hash]") {
...
TEST_CASE("CPO: Hash scalar types deterministic", "[Core.Hash]") {
...
TEST_CASE("CPO: Hash float deterministic", "[Core.Hash]") {
...
TEST_CASE("CPO: Hash enum", "[Core.Hash]") {
    constexpr auto h1 = DoHash(Color::Red);
    constexpr auto h2 = DoHash(Color::Green);
    STATIC_REQUIRE(h1 != h2);
}

TEST_CASE("CPO: Hash string_view matches raw algo", "[Core.Hash]") {
...
TEST_CASE("CPO: Hash struct (reflection-driven)", "[Core.Hash]") {
...
TEST_CASE("CPO: Hash nested struct", "[Core.Hash]") {
...
TEST_CASE("CPO: Hash vec3", "[Core.Hash]") {
...
TEST_CASE("CPO: explicit algorithm Fnv1a32", "[Core.Hash]") {
...
TEST_CASE("CPO: ADL hook takes priority", "[Core.Hash]") {
...
TEST_CASE("Annotation: HashIgnore excludes member from hash", "[Core.Hash.Annotation]") {
...
TEST_CASE("Annotation: HashKey whitelist mode", "[Core.Hash.Annotation]") {
...
TEST_CASE("Annotation: Anno::With overrides algorithm", "[Core.Hash.Annotation]") {
...
TEST_CASE("Annotation: HashOrder controls hash sequence", "[Core.Hash.Annotation]") {
...
TEST_CASE("Combine: non-commutative", "[Core.Hash.Combine]") {
    constexpr uint64_t a = 0x12345678ULL;
    constexpr uint64_t b = 0xABCDEF00ULL;
    STATIC_REQUIRE(Combine(a, b) != Combine(b, a));
}

TEST_CASE("Combine: 32-bit variant works", "[Core.Hash.Combine]") {
...
TEST_CASE("Hasher: deterministic output", "[Core.Hash]") {
...
TEST_CASE("Hasher: Feed chaining deterministic", "[Core.Hash]") {
...
TEST_CASE("Hasher: order matters", "[Core.Hash]") {
    constexpr auto h1 = [] {
        Hasher h;
...
TEST_CASE("Hasher: Feed struct expands members", "[Core.Hash]") {
    constexpr SimpleStruct s{1, 2, 3};
    constexpr auto result = Hasher<>::Of(s);
    STATIC_REQUIRE(result != 0);
}

TEST_CASE("Hasher: 32-bit state variant", "[Core.Hash]") {
...
TEST_CASE("std::hash<vec3> with unordered_map", "[Core.Hash]") {
...
TEST_CASE("std::hash<SimpleStruct> works", "[Core.Hash]") {
...
TEST_CASE("std::hash deterministic", "[Core.Hash]") {
    std::hash<vec3> h;
    vec3 v{1, 2, 3};
    REQUIRE(h(v) == h(v));
}
...
TEST_CASE("Constexpr: CPO folds at compile time", "[Core.Hash]") {
    constexpr auto h = DoHash(vec3{1, 2, 3});
    STATIC_REQUIRE(h != 0);
}

TEST_CASE("Constexpr: Hasher::Of folds at compile time", "[Core.Hash]") {
    constexpr auto h = Hasher<>::Of(1, 2, 3);
    STATIC_REQUIRE(h != 0);
}

TEST_CASE("Constexpr: Combine folds at compile time", "[Core.Hash]") {
