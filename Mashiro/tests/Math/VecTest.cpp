...
TEST_CASE("Vec3 arithmetic matches Eigen", "[Math.Vec]") {
...
TEST_CASE("Vec4 arithmetic matches Eigen", "[Math.Vec]") {
...
TEST_CASE("Vec2 arithmetic matches Eigen", "[Math.Vec]") {
...
TEST_CASE("Hadamard product matches Eigen cwiseProduct", "[Math.Vec]") {
    vec3 a{2.0f, 3.0f, 4.0f}, b{5.0f, 6.0f, 7.0f};
    RequireCloseEigen(a * b, ToEigen(a).cwiseProduct(ToEigen(b)));
}

TEST_CASE("Component-wise division matches Eigen cwiseQuotient", "[Math.Vec]") {
...
TEST_CASE("Dot product matches Eigen", "[Math.Vec]") {
...
TEST_CASE("Cross product (3D) matches Eigen", "[Math.Vec]") {
...
TEST_CASE("Cross product (2D) matches manual formula", "[Math.Vec]") {
    vec2 a{3,4}, b{-1,2};
    auto ea = ToEigen(a), eb = ToEigen(b);
    RequireClose(Math::Cross(a, b), ea.x() * eb.y() - ea.y() * eb.x());
}

TEST_CASE("Norm1/Norm2/NormInf match Eigen norms", "[Math.Vec]") {
    vec3 v{3,-4,12};
    auto ev = ToEigen(v);
...
TEST_CASE("Normalize matches Eigen .normalized()", "[Math.Vec]") {
    vec3 v{3,-4,0};
...
TEST_CASE("Distance matches Eigen (b-a).norm()", "[Math.Vec]") {
...
TEST_CASE("Min/Max match Eigen cwiseMin/cwiseMax", "[Math.Vec]") {
...
TEST_CASE("Abs matches Eigen .cwiseAbs()", "[Math.Vec]") {
    vec3 v{-1,2,-3};
    RequireCloseEigen(Math::Abs(v), ToEigen(v).cwiseAbs());
}

TEST_CASE("Lerp matches Eigen manual lerp", "[Math.Vec]") {
