...
TEST_CASE("MakeTranslation2D matches Eigen Translation2f", "[Math.MathUtils]") {
    vec2 t{3,-7};
...
TEST_CASE("MakeScale2D matches Eigen Scaling2D", "[Math.MathUtils]") {
    vec2 s{2,3};
    Eigen::Affine2f ea = Eigen::Affine2f::Identity();
    ea.scale(ToEigen(s));
...
TEST_CASE("MakeRotate2D matches Eigen Rotation2Df", "[Math.MathUtils]") {
    float angle = 1.5f;
    Eigen::Affine2f ea = Eigen::Affine2f::Identity();
...
TEST_CASE("IdentityAffine2D is upper 2 rows of Identity<mat3>()", "[Math.MathUtils]") {
...
TEST_CASE("Affine2D builders match upper rows of 3x3", "[Math.MathUtils]") {
...
TEST_CASE("InverseAffine2D matches Eigen Affine2f .inverse()", "[Math.MathUtils]") {
    float angle = 0.8f;
    vec2 t{3,-7};
