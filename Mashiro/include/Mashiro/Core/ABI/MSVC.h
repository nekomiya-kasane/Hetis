/**
 * @file MSVC.h
 * @brief MSVC ABI name mangling — `consteval`, reflection-driven, x64.
 *
 * Spec of record: clang's actual emitter (this is the native target ABI).
 * Output for *types* matches `typeid(T).raw_name()` byte-for-byte:
 * - builtin: `.H` (int), `.N` (double), `.D` (char), ...
 * - class Foo in global ns: `.?AVFoo@@`
 * - `ns::Bar`: `.?AVBar@ns@@`
 *
 * Output for functions/variables matches the linker symbol:
 * - free `int foo()`: `?foo@@YAHXZ`
 * - global `int g`: `?g@@3HA`
 *
 * Compression: two fixed 10-slot back-reference tables (name fragments `0-9`, compound types `0-9`), threaded 
 * through the consteval recursion.
 *
 * @ingroup Core
 */
#pragma once
...
namespace Mashiro::ABI::MSVC {
    ...
    namespace Detail {
        ...
        consteval std::string_view BuiltinLetter(ABI::Detail::Builtin b) {
            ...
            switch (b) {
                case B::Void:       return "X";
                case B::Bool:       return "_N";
                case B::Char:       return "D";
                case B::SChar:      return "C";
                case B::UChar:      return "E";
                case B::Char8:      return "_Q";
                case B::Char16:     return "_S";
                case B::Char32:     return "_U";
                case B::WChar:      return "_W";
                case B::Short:      return "F";
                case B::UShort:     return "G";
                case B::Int:        return "H";
                case B::UInt:       return "I";
                case B::Long:       return "J";
                case B::ULong:      return "K";
                case B::LongLong:   return "_J";
                case B::ULongLong:  return "_K";
                // __int128 / unsigned __int128 verified via clang-cl + llvm-nm:
                //   `__int128 g` → `?g@@3_LA`, `unsigned __int128 g` → `?g@@3_MA`.
                case B::Int128:     return "_L";
                case B::UInt128:    return "_M";
                case B::Float:      return "M";
                case B::Double:     return "N";
                case B::LongDouble: return "O";
                case B::NullPtr:    return "$$T";
                default:            return {};
            }
        }
        ...
        consteval char ClassKindTag(std::meta::info t) {
            if (std::meta::is_union_type(t)) return 'T';
            if (std::meta::is_enum_type(t))  return 'W'; // followed by '4'
            auto members =
                std::meta::nonstatic_data_members_of(t, std::meta::access_context::unchecked());
            if (!members.empty() && std::meta::is_public(members[0])) return 'U';
            return 'V';
        }
        ...
        consteval void AppendNameFragment(std::string& out, std::string_view id, State& st) {
            ...
            out += id;
            out += '@';
            if (st.nameBackrefs.size() < 10) st.nameBackrefs.push_back(id);
        }
        ...
        consteval void AppendQualifiedName(std::string& out, std::meta::info t, State& st) {
            ...
            for (auto info : chain) {
                std::string_view id = std::meta::has_identifier(info)
                    ? std::meta::identifier_of(info) : std::string_view{"?A"};
     ...
     *
     * - **Type** → typeid form (`.H`, `.?AVFoo@@`) matching `raw_name()`.
     * - **Variable** → linker symbol `?<name>@@3<type><storage>`.
     * - **Function** → linker symbol `?<name>@@<access><cc><ret><params>@Z`.
     */
    [[nodiscard]] consteval std::string_view Mangle(std::meta::info entity) {
        ...
        if (std::meta::is_type(entity)) {
            ...
            } else if (std::meta::is_lvalue_reference_type(type) ||
                       std::meta::is_rvalue_reference_type(type)) {
                out += 'E';
            ...
            if (!isMember) {
                ...
                if (std::meta::is_virtual(entity))      out += 'U';
                else if (std::meta::is_public(entity))  out += 'Q';
                else if (std::meta::is_protected(entity)) out += 'I';
                else                                    out += 'A';
                out += 'E';
                if (std::meta::is_lvalue_reference_qualified(entity))      out += 'G';
                else if (std::meta::is_rvalue_reference_qualified(entity)) out += 'H';
                out += std::meta::is_volatile(entity)
                           ? (std::meta::is_const(entity) ? 'D' : 'C')
