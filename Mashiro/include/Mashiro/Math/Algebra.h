...
namespace Mashiro {
    ...
    namespace Math::Detail {
        ...
        template <typename T>
        concept HasSubscript = requires(T v) { { v[0] }; };
        ...
        using ScalarTypeOf = typename decltype(ScalarTypeOfImpl<T>())::type;
     ...
     */
    template <typename T>
    concept VectorSpace = AdditiveGroup<T> &&
    ...
    template <typename T>
    concept NormedSpace = VectorSpace<T> &&
        (Math::Detail::HasADLNorm<T> || InnerProductSpace<T>);
    ...
    namespace Math {
        ...
        [[nodiscard]] constexpr T Normalize(T v) {
            ...
            constexpr F kMinSq = F(128) * std::numeric_limits<F>::epsilon()
                               * F(128) * std::numeric_limits<F>::epsilon();
