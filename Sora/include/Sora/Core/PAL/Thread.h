...
namespace Sora {
    ...
    namespace PAL {
        ...
        namespace Traits {
            ...
            inline constexpr size_t kMaxThreadNameLength = Sora::Platform::kIsLinux ? 15 : 63;
         ...
         * @details The identifier is process-local and may be reused after a thread exits. It is suitable for
         * correlating Sora diagnostics with native debuggers, profilers, and Trace Event consumers.
         * @return A nonzero native identifier on supported platforms, or zero when the platform cannot provide one.
         */
        [[nodiscard]] Result<uint64_t> CurrentNativeThreadId() noexcept;
         ...
         * @details The result is an observation, not affinity: the scheduler may migrate the thread immediately
         * afterward.
         ...
         * @details The result describes the current native stack. Code running on fibers or user-mode scheduled stacks
         * must query again after switching execution contexts.
    ...
    namespace Traits {
        ...
        inline namespace PAL {

            using Sora::PAL::Traits::kMaxThreadNameLength;
