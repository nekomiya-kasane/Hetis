...
namespace Sora {
    ...
    namespace Traits {
        ...
        using CallbackTagsOf = typename
            [:Sora::Detail::DeclaredTypeListInfoOf<^^Meta::IsCallbackTagList>(^^std::remove_cvref_t<T>, "Callbacks"):];
    ...
    public:
        ...
        [[nodiscard]] static EventObjectLease Borrowed(void* object) noexcept { return EventObjectLease{object, {}}; }
            ...
            [[nodiscard]] static EventPort& Dispatch(T& object) {
                if constexpr (requires { EventPortOf(object); }) {
                    ...
                    static_assert(std::same_as<std::remove_cvref_t<decltype(object.events)>, EventPort>,
                                  "A member named 'events' must be Sora::EventPort.");
                    return object.events;
                } else {
    ...
    namespace Concept {
        ...
        concept EventParticipant =
            std::is_class_v<std::remove_cvref_t<T>> && Concept::HasEventPort<T> && requires(T& object) {
                { Sora::EventPortOf(object) } -> std::same_as<EventPort&>;
                { Sora::EventLifetimeOf(object) } -> std::same_as<EventObjectLifetime>;
                { Sora::EventRefOf(object) } -> std::same_as<EventObjectRef>;
            };
