 ...
 *
 * Exposes @ref Sora::uint128_t and @ref Sora::int128_t as project-wide names for the
 ...
 * its accumulator as @ref Sora::uint128_t and converts to/from the two-lane @c Uuid via
...
namespace Sora {
    ...
    static_assert(sizeof(uint128_t) == 16, "Sora::uint128_t must be a 16-byte integer");
    static_assert(sizeof(int128_t) == 16, "Sora::int128_t must be a 16-byte integer");
    static_assert(sizeof(uint128_t) * CHAR_BIT == 128, "Sora::uint128_t must be exactly 128 bits wide");
