 ...
 *
 * **Architecture.** Public constants live in `Sora::Math::Const` (see Constants.h).
 ...
 * tables). The public API lives directly in `Sora::Math`.
 *
 * @see Constants.h
 * @ingroup Math
 */
...
namespace Sora::Math {
    ...
    /// @brief Number of precomputed factorial values, covering the polynomial kernels.
    inline constexpr size_t kFactorialTableSize = 32;

    /**
     * @brief Compile-time factorial lookup table generated by a consteval block.
     * @tparam T  Arithmetic result type.
     */
    template<typename T>
        requires std::is_arithmetic_v<T>
    inline constinit auto kFactorialTable = [] consteval {
        std::array<T, kFactorialTableSize> values{};
        values[0] = T(1);
        for (size_t i = 1; i < values.size(); ++i) {
            const T factor = static_cast<T>(i);
            if (values[i - 1] > std::numeric_limits<T>::max() / factor) {
    ...
    /**
     * @brief Factorial with a precomputed static lookup table.
     * @tparam T  Arithmetic result type.
     * @param n   Non-negative integer.
     * @return \f$ n! \f$ converted to `T`.
     */
    template<typename T>
        requires std::is_arithmetic_v<T>
    [[nodiscard]] constexpr T Factorial(T n) {
        ...
        if (static_cast<size_t>(n) < kFactorialTableSize) {
            return kFactorialTable<T>[static_cast<size_t>(n)];
        }

        T value = kFactorialTable<T>.back();
        for (T i = static_cast<T>(kFactorialTableSize); i <= n; ++i) {
            if (value > std::numeric_limits<T>::max() / i) {
                static_assert(std::numeric_limits<T>::has_infinity, "Factorial overflow");
    ...
    namespace Detail {
        ...
        inline constexpr auto kSinPoly = MakePoly<T, kSinTerms<T>>([](int k) -> T {
            long double v = 1.0L / Factorial<long double>(2 * k + 1);
        ...
        inline constexpr auto kCosPoly = MakePoly<T, kCosTerms<T>>([](int k) -> T {
            long double v = 1.0L / Factorial<long double>(2 * k);
            return static_cast<T>((k & 1) ? -v : v);
        });
        ...
        [[nodiscard]] constexpr T CtSqrt(T x) {
            ...
            if constexpr (std::same_as<T, float>) {
                ...
                y = std::bit_cast<double>((std::bit_cast<std::uint64_t>(x) >> 1) + 0x1ff7a3bea91d9b1bULL);
            }
